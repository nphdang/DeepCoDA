...
from keras.layers import Input, Dense, Concatenate, Lambda, Dot
...
for dataset in list(datasets.values()):
    ...
    for run in range(n_run):
        ...
        h = Dense(hidden_dim, activation='relu')(all_z)
        beta = Dense(cascade_level, activation='linear')(h)
        # Decoder
        all_z_beta = Dot(axes=1)([all_z, beta])
        decoder = Sequential([Dense(output_dim, input_dim=output_dim, activation='sigmoid')])
        ...
        if n_run == 1:
            ...
            with open('w_attn_wei_logs_{}_lvl_{}_s0_{}_l1_{}_run{}.csv'.format(dataset, cascade_level, sumzero_lambda,
                ...
                for level_id in range(cascade_level):
                    ...
                    W_z.append(w_zi)
                W_z = np.array(W_z)
            # obtain weights of the Beta layer
            last_log_contrast_layer_id = 2 * (cascade_level - 1)
            beta_layer_id = last_log_contrast_layer_id + 4
            W_beta = model.get_weights()[beta_layer_id]
            print("W_beta={}".format(np.around(W_beta, 2)))
            # save result to file
            with open('w_attn_wei_beta_{}_lvl_{}_s0_{}_l1_{}_run{}.csv'.format(dataset, cascade_level, sumzero_lambda,
                                                                               l1_lambda, run), 'w') as f:
                for w_betai in W_beta:
                    w_betai = np.around(w_betai, 2) + 0.0
                    f.write("{}\n".format(",".join(map(str, w_betai))))
            # obtain mapping between x and all_z
            map_x_all_z = Model(x, all_z)
            W_x_all_z = map_x_all_z.predict(X_train, batch_size=batch_size)
            # save result to file
            with open('w_attn_map_x_z_{}_lvl_{}_s0_{}_l1_{}_run{}.csv'.format(dataset, cascade_level, sumzero_lambda,
                                                                              l1_lambda, run), 'w') as f:
                for w_xz_key, w_xz_val in enumerate(W_x_all_z):
                    w_xz_val = np.around(w_xz_val, 2) + 0.0
                    f.write("{},{}\n".format(X_rowname[w_xz_key], ",".join(map(str, w_xz_val))))
            # obtain mapping between x and beta
            map_x_beta = Model(x, beta)
            W_x_beta = map_x_beta.predict(X_train, batch_size=batch_size)
            # save result to file
            with open('w_attn_map_x_beta_{}_lvl_{}_s0_{}_l1_{}_run{}.csv'.format(dataset, cascade_level, sumzero_lambda,
                                                                                 l1_lambda, run), 'w') as f:
                for w_xbeta_key, w_xbeta_val in enumerate(W_x_beta):
                    w_xbeta_val = np.around(w_xbeta_val, 2) + 0.0
                    f.write("{},{}\n".format(X_rowname[w_xbeta_key], ",".join(map(str, w_xbeta_val))))
            # delete trained network to free memory
            del map_x_all_z, map_x_beta
    ...
    # save result to file
    with open('w_attn_auc_{}_lvl_{}_s0_{}_l1_{}.txt'.format(dataset, cascade_level, sumzero_lambda, l1_lambda), 'w') as f:
...
ax.set_ylabel("AUC")
plt.savefig("w_attn_auc_{}_lvl_{}_s0_{}_l1_{}.pdf".format(args.dataset, cascade_level, sumzero_lambda, l1_lambda), bbox_inches="tight")
plt.close()

# save result to csv file
n_dataset = len(datasets)
with open('w_attn_auc_{}_lvl_{}_s0_{}_l1_{}.csv'.format(args.dataset, cascade_level, sumzero_lambda, l1_lambda), 'w') as f:
